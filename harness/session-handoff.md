...
- 当前计划：后端服务化计划已完成，归档到 `plans/archive/2026-05-24-backend-service.md`
...
- 服务协议：`GET /api/tools` 返回 realtime protocol 与工具 registry；`GET /api/realtime` 下行 `tool_request`，应用端回传 `tool_result`。
- 工具链路：`ASREnded transcript -> 规则版后端 Planner -> tool_request -> app tool_result 或后端 fallback -> 300 ChatTTSText 播报 tool_started/tool_result`。
- 当前工具：`map.open`、`map.close`、`map.set_origin`、`map.set_destination`、`navigation.start`。
...
- 2026-05-24 已运行并通过：`./scripts/harness-check.sh`、`npm run typecheck`、`npm test`、`npm run build`、`npm run smoke:local`。
- `npm test` 当前 4 files / 18 tests passing。
...
- 真实地图/导航服务由“金工小子” app 执行；本仓库已提供 `tool_request` / `tool_result` 协议和 demo auto-ack fallback，但尚未和真实 app 联调。
...
- 进度状态：`harness/feature_list.json` 已同步 F001-F006；`harness/progress.md` 已追加服务化记录；active plan 已归档。
...
1. 让“金工小子” app 按 `GET /api/tools` 返回的协议接入 `/api/realtime`。
2. app 收到 `tool_request` 后执行真实地图动作，并发送 `tool_result`。
3. 联调真实语音、工具回传、音频播放和插话打断；若复现 bug，先点 `Save log` 或保存同等结构化日志。
