...
### 2026-06-14 - 记录金工小子仓库边界耦合债务

- 决策：本轮只记录 DuplexKit 与金工小子 app 的仓库边界耦合问题，不在系统提示词地点补全任务中重构仓库关系。
- 原因：当前紧急目标是修正金工小子语音模型的地点背景；仓库拆分涉及 submodule 方向、测试 fixture 和 app 地图数据所有权，容易扩大范围。
- 否决方案：在提示词修复中同时移动 `roomResolver`、复制地图数据或调整 submodule 布局。
- 后续约束：后续单独开任务修复；DuplexKit 根仓库测试不得依赖 `apps/jingongxiaozi` 或父目录中的 app 源码，app 可通过网络协议或明确的 submodule 依赖使用 DuplexKit。
