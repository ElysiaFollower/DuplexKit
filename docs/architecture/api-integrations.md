...
官方文档说明该接口是一次请求即返回识别结果，支持 WAV / MP3 / OGG OPUS，适合当前分段上传 demo。当前主 demo 只走 `/api/turn`，不再使用浏览器 ASR fallback。
...
因此当前单一路线的阻塞点是 ASR resource grant。解决方式二选一：开通与 `volc.bigasr.auc_turbo` 匹配的极速/flash 识别资源，或后续实现 “Doubao-录音文件识别2.0” 的异步 submit/query provider。
