...
DuplexKit 是原生实时全双工语音后端服务和工具调用原型。应用端负责麦克风采集、音频播放和真实地图界面；后端负责把应用端 WebSocket 桥接到火山实时语音大模型，并在模型完整 assistant response 后解析固定工具声明。
...
- 后端在 `ChatEnded` 后用规则版 Planner 解析语音模型固定工具声明，判断是否触发 demo 工具。
